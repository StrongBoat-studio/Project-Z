...
public class MutantMovement : MonoBehaviour
{
    ...
    [SerializeField] private float _distanceChase, _distanceAttack;
    [SerializeField] private float _secondOfSleep;
    [SerializeField] private float _secondsElapsed;

    private Transform _target;

    private bool _chase=false;
    private bool _zone1=false, _zone2=false, _zone3=false, _zone4=false;
    ...
    private void TimeManipulation()
    {
        ...
        if (_distanceChase<=5 && !_zone1)
        {
            _secondsElapsed -= 2;
            _zone1 = true;
        }

        if (_distanceChase <= 4 && !_zone2)
        {
            _secondsElapsed -= 1;
            _zone2 = true;
        }

        if (_distanceChase <= 3 && !_zone3)
        {
            _secondsElapsed = 0;
            _zone3 = true;
        }
